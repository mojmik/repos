...
namespace mCompWardenManagement
{
    public partial class MainWindow : Window
    {
        ...
        private bool _autoGeneratedFileName;
        private bool _suppressFileNameTracking;
        private string _generatedFileNameSuffix = "";
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                if (string.IsNullOrWhiteSpace(FileNameText.Text))
                {
                    ...
                    if (ok == true)
                        SetFileNameText(dlg.FileName, autoGenerated: false);
                    else
                        return;
                }
        ...
        private void Load_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                try
                {
                    LoadV3IntoEditor(file, useAsTemplate: false);
        ...
        private void LoadTemplate_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                InitialDirectory = @"\\rentex.intra\company\mkavan_upravy\scripts\mCompWarden2",
                Title = "Vyber V3 šablonu",
                CheckFileExists = true,
                CheckPathExists = true,
                Filter = "V3 XML (*.mcw3.xml)|*.mcw3.xml|XML files (*.xml)|*.xml|All files (*.*)|*.*",
                RestoreDirectory = true,
                ReadOnlyChecked = true,
                ShowReadOnly = true
            };
            ...
            try
            {
                LoadV3IntoEditor(openFileDialog1.FileName, useAsTemplate: true);
                MessageBox.Show("V3 šablona načtena. Ulož ji pod novým názvem.");
            }
            catch (Exception ex)
            {
                MessageBox.Show("Chyba při čtení V3 šablony: " + ex.Message);
            }
        }

        private void LoadV3IntoEditor(string file, bool useAsTemplate)
        {
            ...
            if (OuCombo != null)
                OuCombo.SelectedItem = null;
            if (OuInfo != null && useAsTemplate)
                OuInfo.Text = "";

            SetFileNameText(useAsTemplate ? "" : file, autoGenerated: false);
        ...
        {
            if (_suppressFileNameTracking) return;
            _autoGeneratedFileName = false;
        }

        private void MachineNameText_TextChanged(object sender, TextChangedEventArgs e)
        {
            SyncGeneratedFileNameWithCurrentMachine();
        }

        private void V3MachineText_TextChanged(object sender, TextChangedEventArgs e)
        {
            SyncGeneratedFileNameWithCurrentMachine();
        }

        private void SetFileNameText(string value, bool autoGenerated)
        {
            _suppressFileNameTracking = true;
            try
            {
                FileNameText.Text = value ?? "";
                _autoGeneratedFileName = autoGenerated;
            }
            finally
            {
        ...
        private void SyncGeneratedFileNameWithCurrentMachine()
        {
            if (!_autoGeneratedFileName || string.IsNullOrWhiteSpace(_generatedFileNameSuffix))
                return;
            ...
            SetFileNameText(System.IO.Path.Combine(folder, machine + _generatedFileNameSuffix), autoGenerated: true);
        }
        ...
        private void genFileNameButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            _generatedFileNameSuffix = $"-{dt}_{rndTxt}.{ext}";
            SetFileNameText(System.IO.Path.Combine(folder, machine + _generatedFileNameSuffix), autoGenerated: true);
        }
        ...
        private void SetControls(mCompWarden2.CommandSet cmdSet = null, string file = null)
        {
            ...
            SetFileNameText(file ?? "", autoGenerated: false);
        ...
        private void ClearOuBtn_Click(object sender, RoutedEventArgs e)
        {
            ComboBox ouCombo = OuCombo as ComboBox;
            TextBlock ouInfo = OuInfo as TextBlock;
